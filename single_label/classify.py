...
def define_argparse():
    ...
    p.add_argument('--show_probs', action='store_true')
    p.add_argument('--prob_threshold', type=float, default=0)
...
def main(config):
    ...
    with torch.no_grad():
        ...
        probs, indices = y_hats.cpu().topk(config.top_k)

        for i in range(len(lines)):
            sys.stdout.write('%s\t%s\n' % (
                ' '.join([f"{index_to_label[int(indices[i][j])]}({probs[i][j]*100:.3f}%)" if config.show_probs else index_to_label[int(indices[i][j])] 
                            for j in range(config.top_k) if probs[i][j] >= config.prob_threshold]),
                lines[i]
            ))
